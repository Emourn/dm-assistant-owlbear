...
target_path = os.path.join('src', 'components', 'combat', 'CombatBoard.tsx')
...
for i, line in enumerate(lines):
    if 'if (!activeEncounter || combatants.length === 0) return null;' in line:
        null_idx = i
    if 'onClick={() => setShowEndConfirm(true)}' in line:
        end_confirm_idx = i

if null_idx == -1 or end_confirm_idx == -1:
    print("Could not find anchor points")
    exit(1)

pre_lines = lines[:null_idx + 1]
# We back up one line above end_confirm_idx to grab the <button opening tag
post_lines = lines[end_confirm_idx - 1:]

replacement = r"""    const handleReadyActionConfirm = () => {
...
print("Rebuilt files successfully.")
